...
{
	var xStart = Math.floor(pos.x); //math.floor always rounds down
	var xEnd = Math.ceil(pos.x + size.x); //math.ceil always rounds up
	var yStart = Math.floor(pos.y);
	var yEnd = Math.ceil(pos.y + size.y);
	
	//consider the sides, top, bottom of level as walls
			...
			for(var x=xStart; x<xEnd; x++)
				{
					...
					if (fieldType) //if(fieldType && fieldType != "floater") return fieldType; 
					{
						return fieldType;
					}
				}
...
Player.prototype.moveY = function(step, level, keys) {
	...
	var obstacle = level.obstacleAt(newPos, this.size);
	// only allow the player to jump if they are touching some obstacle
	
	//check the if the obstacle is equal to something then change the position
	...
	//if (obstacle == "lava") {
		//this.pos = new Vector (5,5); - when you want to access the player you use this
	//}
